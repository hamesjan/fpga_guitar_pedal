...
static int32_t delay_buffer_L[MAX_DELAY_SAMPLES] = {0};  // Initialize to zero
static int32_t delay_buffer_R[MAX_DELAY_SAMPLES] = {0};  // Initialize to zero
...
void ping_pong_delay(int32_t* out_L, int32_t* out_R, 
    ...
    // Step 7: Output final mixed signals (with clamping to prevent overflow)
    int64_t final_L = wet_L + dry_L;
    int64_t final_R = wet_R + dry_R;
    
    // Clamp output to 24-bit range
    if (final_L > 8388607) final_L = 8388607;
    if (final_L < -8388608) final_L = -8388608;
    if (final_R > 8388607) final_R = 8388607;
    if (final_R < -8388608) final_R = -8388608;
    
    *out_L = (int32_t)final_L;
    *out_R = (int32_t)final_R;
